...
\end{equation}
In section \ref{sec:dmrg} we will variationally optimize ground states within $\mathcal{M}_{\{D_n\}}$, using the \textit{density matrix renormalization group} (DMRG) algorithm. Then, in section \ref{sec:vqpe}, we will construct quasiparticle excitations on top, by optimizing (a parametrized version of) the tangent space vector
...
\begin{equation} \label{eq:emps_orthogonality}
	...
	\langle \psi(\overline{Y}^{[m]}; \overline{A}) \vert \psi(X^{[n]}; A) \rangle = \delta_{nm} ( \overline{Y}^{[n]} \vert X^{[n]} ).
\end{equation}
...
\begin{enumerate}
	...
	\item[(ii)] transfer the boundary for which all $B^{[m < n]}$ already have been applied,
\end{enumerate}
...
\vspace*{1.5em}
\noindent For an MPS quasiparticle excitation \eqref{eq:excitation_ansatz}, a local energy expectation value leads to the double sum 
\begin{equation}
