...
The reduced density matrix $\rho_L = \mathrm{tr}_R \ket{\psi (A)}\bra{\psi (\overline{A})}$ yields the \textit{entanglement entropy}
\begin{equation}
...
\begin{align}
\begin{split}
	\argmin_X \Vert \ket{\varphi} - \ket{\psi(X; A)} \Vert^2 = \argmin_X \big[ & \langle \psi(\overline{X}; \overline{A}) \vert \psi(X; A) \rangle - \langle \psi(\overline{X}; \overline{A}) \vert \varphi \rangle \\
