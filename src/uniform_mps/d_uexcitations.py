...
from .a_umps import TransferMatrix
from .c_vumps import InverseGeometricSum, get_Lh, get_Rh, subtract_energy_offset
...
class VariationalPlaneWaveExcitationEngine:
    ...
    With <phi(p',X'; A)|H|phi(p,X; A)> = 2 * pi * delta(p-p') * <X'|H_eff(p)|X>, the variational
    optimization boils down to diagonalizing H_eff(p) for a few lowest-lying eigenvalues.
    ...
    """
    def __init__(self, umps0, h, p, umps0_tilde=None, tol=1.e-14):
        self.h = subtract_energy_offset(umps0, h, canonical_form=True)
        self.p = p
        self.tol = tol
        self.VL = self.get_VL(umps0.AL)
        self.AL = umps0.AL
        self.Lh = get_Lh(umps0, self.h, canonical_form=True, guess=None, tol=self.tol)
        if umps0_tilde is None:
            self.AR = umps0.AR
            self.Rh = get_Rh(umps0, self.h, canonical_form=True, guess=None, tol=self.tol)
            self.IGS_RL = InverseGeometricSum(umps0.AR, umps0.AL, R=np.conj(umps0.C).T, L=umps0.C.T, \
                                              transpose=True, alpha=np.exp(-1.j * p), pseudo=True)
            self.IGS_LR = InverseGeometricSum(umps0.AL, umps0.AR, R=umps0.C, L=np.conj(umps0.C), \
                                              transpose=False, alpha=np.exp(1.j * p), pseudo=True)
        else:
            AR = self.fix_momentum(umps0.AL, umps0_tilde.AR)
            umps0_tilde.AR = self.AR = AR
            self.Rh = get_Rh(umps0_tilde, self.h, canonical_form=True, guess=None, tol=self.tol)
            self.IGS_RL = InverseGeometricSum(umps0_tilde.AR, umps0.AL, R=None, L=None, \
    ...
    def fix_momentum(AL, AR):
        """Multiply AR with a phase such that T_RL has a positive leading eigenvalue."""
...
class Heff(LinearOperator):
    ...
    def __init__(self, h, p, VL, AL, AR, Lh, Rh, IGS_RL, IGS_LR, tol):
        ...
        self.dtype = reduce(np.promote_types, [h.dtype, VL.dtype, AL.dtype, AR.dtype, \
                                               Lh.dtype, Rh.dtype, IGS_LR.dtype, IGS_LR.dtype])
